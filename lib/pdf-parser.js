    ...
    parseTextLines(lines) {
        ...
        for (const line of lines) {
            ...
            for (let i = endIndex; i > 0; i--) {
                const t = tokens[i];
                // Sanitize: remove commas and dashes/hyphens
                const cleanT = t.replace(/[-,–]/g, '');

                // Allow if it looks like a number after cleaning, or was a placeholder dash
                // We check cleanT.length > 0 to ensure we don't pick up empty strings unless they were explicit dashes
                if ((/^[\d]+(\.\d+)?$/.test(cleanT) && cleanT.length > 0) || t === '-' || t === '–') {
                    if (columnsFound < 7) {
                        dataTokens.unshift(cleanT); // Store the number without the negative sign
                        columnsFound++;
                        dataStartIndex = i;
                    } else {
                        break;
                    ...
                    if (columnsFound === 0) {
                        // Keep going if we haven't found data yet (handling trailing text noise if any)
                    } else {
                        break;
                    }
                }
