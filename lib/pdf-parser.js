    ...
    parseTextLines(lines) {
        ...
        for (const line of lines) {
            ...
            for (let i = endIndex; i > 0; i--) {
                ...
                // If it cleans down to a number, or it is a dash
            ...
            const numericTokens = dataTokens.map(t => {
                if (/^[-\u2013\u2014\u2212]+$/.test(t)) return 0;
            ...
            if (cnt >= 7) {
                // Standard: [Prop] [?] [Stock] [Transit] [Usage] [Closing] [Var]
                proposedQty = numericTokens[cnt - 7];
                rsp = numericTokens[cnt - 5];
                transit = numericTokens[cnt - 4];
            } else if (cnt >= 3) {
                // Sparse: [Prop] [Stock] [Transit] ... (maybe followed by Usage/Var etc)
                // If we have fewer columns, the "Gap" usually disappears.
                // We assume the first 3 numbers found are the core ordering data.
                proposedQty = numericTokens[0];
                rsp = numericTokens[1];
                transit = numericTokens[2];
            } else {
                // Fallback for very broken lines (1 or 2 numbers)
                if (cnt >= 1) proposedQty = numericTokens[0];
                if (cnt >= 2) rsp = numericTokens[1];
            }
