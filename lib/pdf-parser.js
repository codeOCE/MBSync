class PDFParser {
    ...
    parseTextLines(lines) {
        ...
        for (const line of lines) {
            ...
            // Adjusted logic: Assume Left-Aligned for sparse data
            // If we found numbers, they usually start with Proposed, Stock, Transit
            if (dCount >= 3) {
                proposedQty = parseFloat(dataTokens[0]) || 0;
                rsp = parseFloat(dataTokens[1]) || 0;
                transit = parseFloat(dataTokens[2]) || 0;
            } else {
                // Extremely sparse (1-2 cols) - fallback to Right-Aligned 'safe' guess?
                // Or just map to Prop/Stock?
                if (dCount >= 1) proposedQty = parseFloat(dataTokens[0]) || 0;
                if (dCount >= 2) rsp = parseFloat(dataTokens[1]) || 0;
            }
