        ...
        for (const line of lines) {
            ...
            for (let i = endIndex; i > 0; i--) {
                ...
                if (((/^[\d]+(\.\d+)?$/.test(cleanT) && cleanT.length > 0) || isDash) && !hasLetters) {
                    if (columnsFound < 8) { // Increased to 8 to capture more contextual columns if available
            ...
            if (cnt >= 3) {
                ...
                // Left-Aligned Indexing based on fixed column order:
                // Col 0: Proposed Qty
                // Col 1: Order / Transit (Stacked Header) - This is what the user wants for "Transit"
                // Col 2: RSP

                // Index 0: Proposed Qty
                proposedQty = numericTokens[0];

                // Index 1: Transit (The "Order" column)
                if (cnt > 1) transit = numericTokens[1];
                ...
                if (cnt >= 2) transit = numericTokens[1];
            }
